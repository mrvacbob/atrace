...
template<> struct alignas(16) vectorX<float, N>
{
	...
	void zero_w() {s = vsetq_lane_f32(0.f, s, 3);}

	vectorX(T a_, T b, T c, T d = 1) {
		alignas(16) T vals[4] = {a_, b, c, d};
		s = vld1q_f32(vals);
	}
	explicit vectorX(T v) : s(vdupq_n_f32(v)) {if (N!=4) zero_w();}
	explicit vectorX(double v) : s(vdupq_n_f32(static_cast<float>(v))) {if (N!=4) zero_w();}
	vectorX(float32x4_t v_) : s(v_) {}
	vectorX() : s(vdupq_n_f32(0.f)) {}
	vectorX(const V &t) : s(t.s) {}

	T dot(const V &t) const __attribute__((always_inline)) {
		float32x4_t mul = vmulq_f32(s, t.s);
		if (N!=4) mul = vsetq_lane_f32(0.f, mul, 3);
...
#define arithmetic_op(op, neon_fn) \
	V &operator op##=(const V &t) { \
		s = neon_fn(s, t.s); return *this; \
	} \
	V operator op(const V &t) const __attribute__((always_inline)) { \
		return V(*this) op##= t; \
	} \
	V &operator op##=(const T t) { \
		s = neon_fn(s, vdupq_n_f32(t)); return *this; \
	} \
	V operator op(const T t) const __attribute__((always_inline)) { \
		return V(*this) op##= t; \
	}

	arithmetic_op(*, vmulq_f32);
	arithmetic_op(/, vdivq_f32);
	arithmetic_op(-, vsubq_f32);
	arithmetic_op(+, vaddq_f32);

#undef arithmetic_op

	V operator-() const {return V(vnegq_f32(s));}
	...
	V reflect(const V &normal) const __attribute__((always_inline)) {T cosI = dot(normal); return *this - (normal * (cosI * 2.f));}

	void set_min(const V &t) {s = vminq_f32(s, t.s);}
	void set_max(const V &t) {s = vmaxq_f32(s, t.s);}
